...
3. **D1c, CWV optimization passes.** One-lever-at-a-time, measure-before-and-after. Candidates (likely, not prescriptive): image priority/sizing on the hero LCP element, dynamic import for the Recharts surface on `/demo/ecommerce/analytics`, overlay render timing under `useSessionContext`, bundle split evaluation for the ~476KB top chunk surfaced in the baseline. **Target:** LCP, CLS, INP all rated `good` (per the web-vitals library's bucket thresholds) on the three dynamic routes.

4. **Lighthouse full-run gate.** `npx lighthouse` against the three dynamic routes; JSON reports under `docs/perf/lighthouse-<date>-<route>.json`. Targets: Performance ≥90, Accessibility ≥95, Best Practices ≥95, SEO ≥95 (the SEO readouts feed 10d's SEO deliverable).
