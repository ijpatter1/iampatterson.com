...
**Phase 10, Polish, Performance & Launch Prep — NOT STARTED.** Phase 10a (Framework Currency) shipped 2026-04-23 as PR #40 (`1a00490`) — the site now runs on Next.js 16.2.4 + React 19.2.5 + Node 20.9+ + ESLint 9 flat config, and the D2 `after()` migration closed the Phase 9F D9 Vercel fire-and-forget durability gap on the embedded Metabase dashboard. D3 scope expanded inline to clear all 31 `eslint-plugin-react-hooks@7` React-Compiler violations via 3 new shared hooks (`useClientMount`, `usePrefersReducedMotion`, `useSessionId`), ref→state conversions, lazy init patterns, and justified disables — rules now at default severity (error). 11 automated Playwright UAT scenarios + 1 manual `[prod-cold]` Metabase durability check all green. Test suite 1164 → 1187. **Next up: Phase 10 body work (Core Web Vitals, mobile testing, SEO, security review, anonymous ID first-party cookie). 9 deliverables; CWV + perf optimization is the natural first cut since Phase 10a's preflight cleanup targeted the same patterns (`set-state-in-effect`, `refs`).**
