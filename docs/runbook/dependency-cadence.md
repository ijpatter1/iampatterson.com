...
An update that cannot be taken yet stays open and red, or — where leaving it open
would mean merging a break — is closed with the reason recorded here (#105 set that
precedent). Either way it is never `ignore`d in
...
A closed one costs more — with no `ignore`, Dependabot reopens it on the first of the
month, so a row that does not say why it was closed buys the same triage twice.
...
| `@types/node` → 26 — the three services | **Nothing here can use it, and merging it would break a passing check.** Vercel offers **24.x (default), 22.x and 20.x only** — Node 26 is neither available nor announced — so the site's runtime cannot move whatever Node's own LTS date says, and moving the services' types ahead of a runtime that cannot follow buys nothing. `scripts/verify-node24.sh` asserts each service's `@types/node` major equals the major derived from the service Dockerfiles, which it requires to agree; `^26.5.0` would fail that, and the script runs at operator time, so the breakage would be silent until someone reran it by hand. The concrete risk, stated rather than hand-waved: `@types/node@26.5.1` declares `ffi`, `quic` and `vfs`, all `ERR_UNKNOWN_BUILTIN_MODULE` on Node 24. Nothing imports them, and a phantom builtin throws at **module load**, so the failure mode is loud in CI rather than silent in production. | Vercel shipping Node 26, then the row below. Check with the [supported versions](https://vercel.com/docs/functions/runtimes/node-js/node-js-versions) page. Closed three times so far (#129, #135, #139). | 2026-09-12 |
| `node` → 26 — the three service Dockerfiles | Nothing upstream. `engines.node` is `24.x` and `tests/unit/infra/runtime-currency.test.ts` asserts every Docker stage is `node:24-slim`, so the bump is red on arrival by design — and green-looking in CI, because nothing in the root suite rebuilds the images. Node 26 is Current, not LTS. | **Vercel is the binding constraint, not the LTS date.** Vercel offers 24.x, 22.x and 20.x only, so the site cannot move to 26 today; Node 26 reaches LTS 2026-10-28, and Node 24 is supported to 2028-04-30 (maintenance from 2026-10-20), so there is no pressure to go early. When Vercel ships 26, take it as one migration: **the three Dockerfiles, `engines.node`, the four `@types/node` ranges, and `tests/unit/infra/runtime-currency.test.ts`.** `scripts/verify-node24.sh` needs no edit — since 2026-09-12 it derives the expected major from the three service Dockerfiles (refusing to run if they disagree) and checks every other surface against it, including the Homebrew toolchain path it puts on `PATH`. It is the only surface that derives: the number is still written down in `.github/workflows/test.yml` and `infra-reconcile.yml` (`NODE_VERSION`), `infrastructure/cloud-run/claudish-proxy/src/dockerfile.test.ts`, `docs/uat/phase-14-uat.sh`, `CLAUDE.md`, `docs/ARCHITECTURE.md` and `docs/runbook/vercel-build-failing.md`, none of which this checklist used to name. Closed three times so far (#63, #64, #65). | 2026-09-12 |

Four of the five rows are not security exposures: `typescript` is a devDependency
and compiles away, and the two Node rows are compatibility blocks — a runtime
Vercel does not offer yet, and the typings for it. **One row is.** `extract-zip`
is two high-severity advisories with no fix in existence. (`qs` was the other
until 2026-09-12, when express 5 made the pin redundant and it was removed.)
A blocked **runtime** or **framework** row is the case that matters, because
those carry external end dates — which is what step 1 exists to catch.
