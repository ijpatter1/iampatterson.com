...
- ⬜ Session State data model — `SessionState` shape in `sessionStorage` at `iampatterson.session_state`; single listener on the data-layer source; coverage denominator derived from the `DataLayerEvent` union **from day one — no hardcoded 16 or 22 magic numbers anywhere in the coverage logic, regardless of which deliverable (4 or 9) lands first**; 16 at baseline, grows to 22 automatically once deliverable 9 lands; subscription + leadgen event types stay in the denominator; monotonic demo-stage progress
