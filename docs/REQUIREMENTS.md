...
**Deferred-not-fixed (10c Pass-1 evaluator follow-up):** A fourth operational defect surfaced during the Pass-1 evaluator review and is being explicitly deferred rather than absorbed: the data-generator's `state.isRunning` gate in `infrastructure/cloud-run/data-generator/src/server.ts` is a single global flag, so any in-flight `/generate` or `/backfill` blocks all others. The 20-min staggered Cloud Scheduler cadence (`:00`/`:20`/`:40` ET per model) is the band-aid — it gives 13-17 min of headroom over the observed 3-7 min `/generate` runtime. Acceptable until/unless a model regresses past 20 min runtime. Structural fix (per-model state-keyed gate, or a queue) belongs to Phase 11 ops. Tracked here so the next evaluation pass doesn't re-flag.
...
9. **Terraform-based GCP infrastructure-as-code:** Adopt Terraform as the canonical, declarative source of truth for all Terraform-supported GCP resources in the project, replacing the current sprawl of imperative one-shot provisioning scripts. Terraform configs live under `infrastructure/terraform/` with remote state in a versioned, state-locked GCS backend bucket. *(Quality-improvement revision 2026-06-03: this deliverable was originally "Declarative infra deploy pipeline — generic hand-written reconcilers." The reconciler approach is retained only for the one surface Terraform genuinely cannot reach — GTM container config — and replaced by Terraform everywhere else. Rationale below.)*
   ...
   **(a) In scope — Terraform-managed:**
   ...
   - **Cloud SQL:** `metabase-app-db` (Postgres 15, db-f1-micro) instance + database + user + backups/PITR
   - **Metabase LB topology:** reserved static IP, serverless NEG, backend service, URL map (including the `/app/*`, `/api/*`, `/embed/*` → IAP/non-IAP backend split), target HTTPS proxy, global forwarding rule, Google-managed SSL cert, IAP OAuth client + `roles/iap.httpsResourceAccessor` allowlist. This is the exact drift family behind Phase 9F's IAP-blocked `/app/*` incident (diagnosed 2026-04-22): a silently-incomplete LB route spec that first-time users discovered via production. A `terraform plan` no-op is the standing proof the route surface is complete
   - **Cloud Scheduler:** the three staggered data-generator jobs (`:00`/`:20`/`:40`)
   - **Cloud Storage:** the AI-export bucket + lifecycle rules (Phase 11 D5)
   - **Secret Manager:** secret *resource shells* only — secret *values* stay out of state (managed out-of-band, with `ignore_changes` on versions)
   - **Cloud Monitoring / alerting / uptime resources:** the Phase 11 D1–D3 dashboards, alert policies, and uptime checks, authored as Terraform as those deliverables land

   **(b) Carve-outs — deliberately NOT Terraform:**
   - **GTM web + sGTM container configuration** — Google ships no Terraform provider for GTM container entities (triggers, tags, variables, versions). This surface stays declarative-but-reconciler-driven: `infrastructure/gtm/reconcile.js` diffs `infrastructure/gtm/web-container.json` (plus the sGTM equivalent) against the live "Default Workspace", applies adds/updates (deletes behind explicit `--allow-deletes`), versions, and publishes. This is the original deliverable-9 GTM reconciler, preserved intact. Note the split: the Cloud Run service that *runs* sGTM is Terraform-managed; the container *config* it serves is reconciler-managed
   - **Dataform model SQL** — already git-managed and synced to the `dataform` branch via GitHub Action. Terraform may own the Dataform release/workflow-config *resource*, but not the model definitions
   - **Vercel frontend** — out of scope, deliberately cross-provider (see the Deployment section). The Next.js app deploys via Vercel's own pipeline; `next.config.mjs` redirects deploy with it

   **(c) Brownfield import, not greenfield:** every resource above already exists and is serving production. The first Terraform pass is an *import* exercise (`import {}` blocks / `terraform import`) that reconciles state against live resources until `terraform plan` reports a clean no-op — **not** a destroy-and-recreate. A destructive plan touching the LB, IAP, SSL cert, or Cloud SQL resources is a release blocker, not an acceptable convergence step.

   **(d) CI/CD (`.github/workflows/infra-terraform.yml`):** on PRs touching `infrastructure/terraform/**`, run `terraform plan` and post the plan output as a PR comment; on merge to `main`, run `terraform apply` behind an `infra-production` GitHub environment with manual approval. Auth via Workload Identity Federation (no service-account key in repo secrets). The GTM reconciler keeps its own dry-run-on-PR / apply-on-merge job triggered by `infrastructure/gtm/**`. Existing integration pins (`tests/integration/web-container-spec.test.ts` for GTM; a new state-vs-live check for the LB URL-map path matchers) remain the red/green gates ensuring spec and runtime agree. Retires `infrastructure/gtm/deploy-phase6.js` and the `infrastructure/metabase/{deploy,setup-cloudsql,setup-domain,setup-iam,setup-iap,upgrade}.sh`, `infrastructure/pubsub/setup.sh`, and `infrastructure/bigquery/setup.sh` one-shots as each resource reaches Terraform (or, for GTM, reconciler) parity.

   **Rationale for the technology change (was: hand-written generic reconcilers):** GCP has first-class Terraform provider coverage for nearly the entire footprint, so hand-writing reconcilers for Cloud Run / Pub/Sub / BigQuery / LB / Cloud SQL / IAM would reinvent the state-diffing, drift-detection, and dependency-ordering Terraform already does well. Terraform is also the IaC tool prospects are most likely to recognize, making it stronger portfolio signal than a bespoke reconciler. The reconciler pattern is kept only where Terraform genuinely can't reach (GTM containers). Still motivated by the same two Phase 9F drift incidents: the `remove_from_cart` GTM trigger/tag gap (schema extended, allow-list not followed — caught by the GTM reconciler carve-out) and the Metabase URL-map missing `/app/*` path (LB setup runbook didn't capture the full Metabase asset-path surface — caught by Terraform-managed URL-map state).
...
1. **`web_vital` sGTM trigger + GA4 event tag.** Added 2026-04-24 by Phase 10b D1a. The `WebVitalsReporter` (`src/components/scripts/web-vitals-reporter.tsx`) emits `web_vital` events to `window.dataLayer` today, but there is no GTM web-container trigger + GA4 event tag to route them through sGTM to `iampatterson_raw.events_raw`. Until this wiring lands, real-user Core Web Vitals distribution cannot be queried from BigQuery, and the Phase 10b D1c mobile-perf targets remain deferred (see `docs/PHASE_STATUS.md` Phase 10b carry-forward block). **Unblocks:** real-user CWV field data → mobile perf optimization passes → closing the Phase 10b D1c mobile targets. **Depends on:** Phase 11 D9 (Terraform-based GCP IaC) — specifically the GTM-container reconciler carve-out (D9.b), which Terraform deliberately does not replace. Once the reconciler lands, extend `infrastructure/gtm/web-container.json` with (a) a custom-event trigger on `event == 'web_vital'`, (b) a GA4 event tag with `metric_name`, `metric_value`, `metric_rating`, `metric_id`, `navigation_type` parameters, and (c) routing through the existing sGTM BigQuery destination. Pattern precedent: the existing `ce - remove_from_cart` / `GA4 - remove_from_cart` pair landed during Phase 9F follow-up work (pinned by `tests/integration/web-container-spec.test.ts:65-102`) is the exact shape — copy that and swap the event name + parameter map.
