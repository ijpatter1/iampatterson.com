...
with open(out_path, 'w', encoding='utf-8') as o:
    for repo in REPOS:
        ...
        while n < per_repo and page <= 40:
            url = f"repos/{repo}/issues/comments?sort=created&direction=desc&per_page=100&page={page}&since=2019-01-01T00:00:00Z"
            # walk from the cutoff backwards: use 'created' desc with a since floor; filter by created_at <= cutoff
            r = subprocess.run(['gh', 'api', url], capture_output=True, text=True)
            if r.returncode != 0:
                print(f'  {repo} page {page}: {r.stderr.strip()[:80]}'); break
            items = json.loads(r.stdout)
            if not items: break
            for c in items:
                if c.get('created_at', '9') > CUTOFF: continue
        ...
        print(f'{repo}: {n}'); kept += n
print('total comments kept:', kept)
