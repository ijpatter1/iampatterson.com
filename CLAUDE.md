...
- **Review gate is `/guv:eval`**, not the legacy `@evaluator` pair. Ultrareview (`/code-review ultra`) had three free runs per account, one-time, and **all three are spent** — the last on 2026-09-09 against PR #75. Don't offer it as a free option; the local pair (`code-review` at `review.level` plus `guv:reviewer` by name) is the gate. Keep PRs under 8,000 changed lines regardless.
- **Permission grants belong in the tracked `.claude/settings.json`.** `settings.local.json` is gitignored (`.gitignore:133`), and `git worktree add` checks out tracked files only — so a grant that lives there is invisible to every worktree-isolated subagent, including `guv:reviewer`. That is what made subagents prompt constantly for `Bash(bash:*)`, which guv needs because its own scripts run as `bash …/script.sh`. Only genuinely machine-specific settings go in the local file. `bash-guard` also blocks a *commit message* quoting the patterns it scans for; write the message to a file and use `git commit -F`.
- **GTM publish is not gated on drift.** Every merge touching `infrastructure/gtm/**` mints a container version even when the reconciler reports `no drift` — version 11 was published from exactly that. Never read a high version number as evidence something changed; identify a known-good version by its name (`ci: <sha> on <date>`) and commit, per `docs/runbook/infra-apply-damaged-production.md`.
