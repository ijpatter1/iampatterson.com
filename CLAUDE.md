...
**Phase 10a, Framework Currency (Next.js 14 → 16) — IN PROGRESS on `phase/10a-framework-currency`. D1, D2, D3 shipped this session. Hop 1 (Next 14.2.35 → 15.5.15 + React 18 → 19, `a6f8bae`) applied the async Request API codemod to one call site (the seven-file scope in REQUIREMENTS.md turned out to be stale as the prior handoff warned). D2 (`2bb6d1a`) migrated both keep-warm Server Component invocations to `after(() => warmMetabaseDashboard())` from `next/server`, retiring `warmMetabaseDashboardFireAndForget` entirely; closes the Phase 9F D9 Pass-2 Tech Important on Vercel fire-and-forget durability. Hop 2 (`42e6481`) bumped to Next 16.2.4, raised `engines.node` to ≥20.9.0, migrated from `next lint` to the ESLint 9 flat config via the `next-lint-to-eslint-cli` codemod, and disabled five net-new `eslint-plugin-react-hooks@7` "React Compiler" advisory rules (immutability, purity, refs, set-state-in-effect, react/use) as Phase 10 polish tech debt. Warnings sweep (`c76f9bb`) closed out all surfaced lint noise; `npm run lint` now clean at 0/0 on the wider repo scope. Test suite 1164 → 1165 (+1 net). Build clean on Next 16. Remaining: D4 (third-party dep audit — recharts / @radix-ui / @testing-library/react / framer-motion declare React 18 peers, runtime green under React 19), D5 (test-suite stabilization — effectively closed by the green baseline), D6 (build-plus-smoke gate including manual browser smoke across the site), D7 (doc currency — this CLAUDE.md update + ARCHITECTURE.md + session handoff). Branch has 4 commits; next session's first job is the browser smoke pass, then dual-evaluator review and release cut.**
