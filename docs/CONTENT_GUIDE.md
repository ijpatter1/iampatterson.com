...
- Contact
- [Under-the-Hood Toggle — persistent, always visible]
...
Built with the same stack I sell. [Look under the hood to see how →]
...
Interact with any demo, then look under the hood to watch your own session data propagate through every layer of the stack in real time.
...
What you'll see under the hood:
...
**Order confirmation message:** "Thanks for your (simulated) order! This purchase event just fired through sGTM, landed in BigQuery, and updated the dashboards. Look under the hood to see exactly what happened."
...
**Post-signup confirmation:** "Your trial is active! In a real subscription, you'd receive your first box within 5-7 days. In this demo, we've just fired a trial_signup event through the full stack. Look under the hood to see your signup flow through consent, sGTM, BigQuery, and into the cohort models."
...
**Post-submission confirmation:** "Thanks for your inquiry! In a real implementation, this lead just entered a qualification pipeline — AI-scored, categorized by partnership type, and attributed to the channel that brought you here. Look under the hood to see the full journey from your first page view to qualified lead."

---

## Under-the-Hood Contextual Content

### Consulting Pages — Overlay Annotations

When a visitor activates the under-the-hood view on any consulting page, brief annotations explain what's happening at each instrumentation point:
...
When a visitor activates the under-the-hood view on a demo page, the narrative flow explains the full pipeline for each event type:
...
**Proof over claims:** Never say "we deliver great results." Show the result. The under-the-hood mechanism is the embodiment of this principle — instead of claiming the stack works, you show it working.
