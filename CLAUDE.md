...
**Phase 9E — Navigation & Overlay Pivot** (implementation 8/9 dev-complete: D1, D2, D3, D4, D5, D6, D7, D9 shipped; D8 deferred by default per spec. The pivot reframes the site as "one world in two states" — `SessionPulse` is the primary nav affordance, the overlay opens to a new default Session State tab, the homepage pipeline section is now a progressive bleed-through reveal anchored to scroll position with four CRT sibling layers + tier state machine + flicker scheduler (D5), the Demos section has collapsed to a single ecommerce section, and the subscription + lead gen demos have been removed from the site with 301 redirects. **9E ships jointly with Phase 9F** — 9F rebuilds the ecommerce demo's Tier 2/3 reveal mechanics to the native-reveal pattern language; the two phases are authored as separate blocks but release as one branch cut. Phases 9C and 9D are cancelled; the subscription and lead gen demos will be reintroduced in later, separately-numbered phases after 9F ships. Full design spec: `docs/UX_PIVOT_SPEC.md`. Pipeline redesign reference implementation: `docs/input_artifacts/design_handoff_pipeline/`.)
