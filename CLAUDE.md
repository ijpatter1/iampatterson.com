...
**Phase 9E — Navigation & Overlay Pivot — dev-complete (9/9)**. All deliverables shipped: D1 (nav pivot + NavHint), D2 (overlay restructure), D3 (Session State tab), D4 (Session State data model), D5 (pipeline progressive bleed-through reveal), D6 (homepage Demos rebuild), D7 (sub + leadgen removal + 301s), D8 (contact form session-state ride-along), D9 (nav + Session State analytics). The pivot reframes the site as "one world in two states" — `SessionPulse` is the primary nav affordance, the overlay opens to a new default Session State tab, the homepage pipeline section is a progressive bleed-through reveal anchored to scroll position with four CRT sibling layers + tier state machine + flicker scheduler, the Demos section has collapsed to a single ecommerce section, and the subscription + lead gen demos have been removed from the site with 301 redirects. **9E ships jointly with Phase 9F** — 9F rebuilds the ecommerce demo's Tier 2/3 reveal mechanics to the native-reveal pattern language; the two phases are authored as separate blocks but release as one branch cut. Phases 9C and 9D are cancelled; the subscription and lead gen demos will be reintroduced in later, separately-numbered phases after 9F ships. Full design spec: `docs/UX_PIVOT_SPEC.md`. Pipeline redesign reference implementation: `docs/input_artifacts/design_handoff_pipeline/`.)
