...
>
> Last updated: 2026-04-20, session-2026-04-19-029 (Phase 9E Deliverables 4 + 3 dev-complete. Deliverable 4 shipped with 4-pass dual-evaluator convergence. Deliverable 3 shipped: SessionStateTab UI with all seven sections + one-shot typewriter + [SKIPPED] funnel status + portal descriptors, overlay tab integration (not yet default — d2) with dedup'd session_state_tab_view emission, coverage milestone emission with sessionStorage memoization. Deliverables 1-2, 5-8 remain.)
...
- ⬜ Header navigation pivot — remove conventional nav; SessionPulse as primary nav affordance; hover affordance + `NAV · UNDER THE HOOD` label; first-session hint pulse ring (homepage-entry-scoped, ~3s idle trigger / ~10s auto-clear, dismissal surface aligned with `NavHintDismissedEvent.dismissal_mode` enum; reduced-motion fallback); footer carries conventional nav on every page; remove MobileSheet
...
- ⬜ Remove subscription and lead gen demos from the site — delete `/demo/subscription` + `/demo/leadgen` routes, pages, layouts, dashboards, signup flows, account dashboards; remove subscription/leadgen data libraries, dashboard components, partnership form; reduce footer demo links to ecommerce; simplify or remove `DemoFooterNav`; keep subscription/leadgen event types in `src/lib/events/schema.ts` for Session State denominator; wire 301 redirects in `next.config.js` with explicit `permanent: true` flag (`/demo/subscription/:path*` → `/?rebuild=subscription#demos`, `/demo/leadgen/:path*` → `/?rebuild=leadgen#demos`); Demos section surfaces a one-line honesty banner when the `rebuild` param is present
