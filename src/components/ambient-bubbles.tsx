    ...
    >
      {bubbles.map((bubble) => (
        <div
          key={bubble.id}
          role="status"
          className="animate-bubble-rise rounded-full border border-sage-700/30 bg-neutral-900/80 px-3 py-1.5 text-xs font-mono text-neutral-100 backdrop-blur-sm"
        >
          {bubble.eventName}
        </div>
      ))}
