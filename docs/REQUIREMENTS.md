...
6a. **Metabase dashboards as code** — Build a single "E-Commerce Executive" dashboard in the Metabase instance (Phase 9B-infra) with six questions backed by the BigQuery mart tables from Phase 5:
     ...
     1. **Funnel conversion by channel** — stacked bar, sessions → product_view → add_to_cart → purchase rates grouped by `utm_source`. Source: `mart_session_events`.
     ...
     4. **Revenue share by channel (30d)** — donut chart of attributed revenue by `utm_source`. Source: `mart_channel_attribution`.
