...
**Carried into Phase 13 from 12.4 (2026-09-04).** The nightly Dataform `production` workflow has completed FAILED every night for a month on one action, `iampatterson_assertions.assert_volume_anomaly` ("expected zero rows"); the other 19 actions succeed. The assertion counts days with no events across the full data range, and every empty day is a weekend, which the weekday-only generator produces by design: 6 missing days in 61, a gap rate of 0.098 against its 0.10 threshold, which is why it failed 28 nights and passed 2. The assertion is corrected as a scoped change rather than a deliverable; 13.5's "Dataform assertion failure" entry describes the alert that now catches it.

**Errata on the sealed Phase 12 (2026-09-04).** Two acceptance clauses were not met as written, and Phase 12 is complete and immutable, so they are recorded here rather than corrected in place. 12.2's acceptance asks for the rehearsal to be "recorded in a manual task card"; it was scripted instead (`apply.sh rehearse`) and recorded in `docs/verification/`, per the standing rule that nothing a machine can verify goes on a card — the same substitution 12.1's acceptance was amended to allow before that phase closed. 12.3's acceptance asks that each of the four 24-hour queries "returns the injected failure from the 12.2 rehearsal"; one rehearsal on one uptime check cannot inject a failure into four services, so the clause was not satisfiable as written, and the data-generator query correctly returns zero entries. The lesson for Phase 13 is to amend an acceptance clause while its phase is still open, before the last deliverable is marked complete.
