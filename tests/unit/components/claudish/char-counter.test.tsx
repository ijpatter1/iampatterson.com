 ...
 * "412 / 1,200 · 9 em dashes" — the counter shows the REAL cap (user
...
describe('CharCounter', () => {
  it('renders count, cap, and em-dash tally with locale thousands separators', () => {
    ...
    expect(screen.getByText('439 / 1,200 · 9 em dashes')).toBeInTheDocument();
  });

  it('singularizes a single em dash', () => {
    render(<CharCounter text={'word — word'} />);
    expect(screen.getByText('11 / 1,200 · 1 em dash')).toBeInTheDocument();
  });

  it('omits the em-dash tail entirely at zero', () => {
    render(<CharCounter text={'plain text'} />);
    expect(screen.getByText('10 / 1,200')).toBeInTheDocument();
