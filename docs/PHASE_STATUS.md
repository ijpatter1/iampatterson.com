...
- ⬜ (6b) Confirmation-page signed embed — 3 live Metabase iframes inline on `/demo/ecommerce/confirmation` below the order details (daily revenue → funnel → AOV ordering), each with short narrative prose tying the chart back to the visitor's just-placed order. Signed by a Next.js Server Component using `MB_EMBEDDING_SECRET_KEY` + card IDs from `METABASE_EMBED_CONFIG` (10-minute HS256 JWTs via `jsonwebtoken`). Non-embeddable three (ROAS, Revenue share, LTV) live in the overlay's Dashboards tab on the confirmation route as summary cards with deep-links into the IAP-gated `/dashboard/2`. Replaces deliverable 5's static Tier 3 mocks.
- ⬜ Services page cross-links — "See it live →" link in the Tier 2 and Tier 3 summary boxes on `/services`: Tier 2 → `/demo/ecommerce`, Tier 3 → `/demo/ecommerce/confirmation` (pre-filled demo order so the inline BI embeds are meaningful on arrival).
