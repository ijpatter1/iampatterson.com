...
- **10c Voice & Data Honesty** 🔄 D1 + D2 + ops follow-ups shipped 2026-04-24 on branch `phase/10c-voice-and-data-honesty` (cut from `phase/10b-core-web-vitals` HEAD; will rebase onto main after 10b merges). Cat-themed campaigns + UTM variants + COMPANY_NAMES rotated to dog-themed equivalents; brand-vocab regression pin walks both `profiles.ts` and `engines/leadgen.ts`. BQ scrubs: 7,205 events.utm_campaign + 87 ad_platform rows remapped, 3,262 future-dated subscription rows deleted (lifecycle endDate clamp + UTC date math fix), 90 duplicate ad_platform rows deduped from a 6-day overlap spike. `ad_platform_raw` upserts now use `MERGE` so the hourly business-hours scheduler is idempotent. Cloud Run revision `data-generator-00007-b7h`. D3-D5 voice audit deferred to a separate session per the original plan (D2-scrub-first then voice).
