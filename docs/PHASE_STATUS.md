...
- ⬜ Session State data model — `SessionState` shape in `sessionStorage` at `iampatterson.session_state`; single listener on the data-layer source; coverage denominator derived from the `DataLayerEvent` union (16 at baseline, grows to 22 once deliverable 9 lands); subscription + leadgen event types stay in the denominator; monotonic demo-stage progress
