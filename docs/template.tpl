...
[
  {
    "type": "LABEL",
    "name": "label1",
    "displayName": "The table that you want to write data to needs to be created beforehand in the Google Cloud Console. Otherwise, this tag will fail."
  },
  {
    "type": "TEXT",
    "name": "tableId",
  ...
  },
  {
    "type": "LABEL",
    "name": "labelEditData",
    "displayName": "- override existing fields by specifying a new value for a key in the event data (e. g. page_location)",
    "enablingConditions": [
      {
        "paramName": "writeableData",
        "paramValue": "fullEventData",
  ...
  {
    "type": "LABEL",
    "name": "labelDeleteData",
    "displayName": "- delete existing fields by specifying an field without a value (leave the field blank)",
    "enablingConditions": [
      {
        "paramName": "writeableData",
        "paramValue": "fullEventData",
  ...
  },
  {
    "type": "LABEL",
    "name": "label4",
    "displayName": "only write this data   ⤵️",
    "enablingConditions": [
      {
        "paramName": "writeableData",
        "paramValue": "custom",
...
scenarios:
  - name: existing event key is removed from default event data
    code: |-
      const mockData = {
      ...
      for (let i = 0; i < mockData.customData.length; i += 1) {
        ...
        if (elem.fieldValue) {
          ...
          Object.delete(testEvent, elem.fieldName);
        }
      }
  ...
  - name: existing event key is changed in default event data
    code: |-
      const mockData = {
        tableId: "test-project.test_dataset.test_table",
        writeableData: "fullEventData",
        ...
        customData: [{
            fieldName: "event_name",
            fieldValue: "changed event value"
          }],
      };
      ...
      assertApi('logToConsole').wasCalledWith("----testLog: connectionInfo = " + testConnectionInfo);
      assertApi('logToConsole').wasCalledWith("----testLog: rows = " + testRows);
  - name: new event key is added to default event data
    code: |-
      const mockData = {
        tableId: "test-project.test_dataset.test_table",
        writeableData: "fullEventData",
        addTimestamp: false,
        customData: [{
          fieldName: "some_new_key",
          fieldValue: "some new value"
          }],
      };
