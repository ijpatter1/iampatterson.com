...
- ⬜ Nav & Session State analytics (accelerated from Phase 10) — six new events added to `src/lib/events/schema.ts` + `DataLayerEvent` union: `nav_hint_shown`, `nav_hint_dismissed` (`dismissal_mode`, disjoint + exhaustive), `session_pulse_hover` (desktop-only, 60s debounce), `session_state_tab_view` (`source`), `portal_click` (`destination`), `coverage_milestone` (`threshold`). Plus a `click_cta` / `cta_location` audit with closed enum (7 values): `session_pulse | portal_services | portal_about | portal_contact | contact_cta_threshold | pipeline_watch_it_live | footer_under_the_hood` — enforced at the type level via the `CtaLocation` union in schema.ts. Denominator grows 16 → 22; new events surface as chips in the Session State coverage bar
