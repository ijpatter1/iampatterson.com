...
> **Status: in progress (3 of ~10 modules imported, 2026-06-04).** Foundation (backend, providers, project services, service accounts), the Metabase LB/IAP topology, and the five Cloud Run services are authored and brownfield-imported into remote state, each converged to a verified `terraform plan` no-op via local runs (CI enforcement of the no-op is pending the WIF setup below). Remaining modules (Cloud SQL, Pub/Sub, BigQuery, Cloud Scheduler, Secret Manager, IAM bindings, monitoring) are not yet imported; until each lands, the corresponding resource is still provisioned by the imperative scripts under `infrastructure/`. See `infrastructure/terraform/IMPORT_INVENTORY.md` for the live census and per-module status.
...
- **GCP backend → Terraform.** Cloud Run (sGTM, event-stream, data-generator, Metabase), Pub/Sub (topic + push subscription; a dead-letter topic + subscription is *planned net-new, decision pending* — none exists live, see the IMPORT_INVENTORY delta note), BigQuery (the three datasets + table schemas from `infrastructure/bigquery/*.json` + partition/cost controls), Cloud SQL (`metabase-app-db`), the full Metabase load-balancer topology (static IP, serverless NEG, backend service, URL map with the `/app/*`·`/api/*`·`/embed/*` IAP/non-IAP split, HTTPS proxy, forwarding rule, managed SSL cert, IAP OAuth client + accessor allowlist), Cloud Scheduler (the three staggered data-gen jobs), a GCS AI-export bucket + lifecycle (*also planned net-new, decision pending* — no such bucket is live today), Secret Manager secret *shells* (values out of state), service accounts + IAM bindings, and the Phase 11 D1–D3 monitoring/alerting/uptime resources as they land.
