...
**`apply.sh` does not work until it authenticates through IAP.** Its requests now get IAP's 302 or 401, and `lib/metabase_client.sh` treats a 302 as success, so a run fails with a jq parse error or an empty "Authenticated to Metabase as" line rather than an IAP message. A 401 here is not a bad API key; do not regenerate the key in response. Tracked in the control plane's backlog.
