...
CLD3's shape: hashed character n-gram fractions (orders 1-4) into small embeddings, averaged, one ReLU layer (48 units), softmax over two classes. 27,026 parameters, quantized int8 symmetric per-tensor, shipped as 36,972 bytes of base64 JSON. Inference is dependency-free TypeScript; measured in Node on 2026-09-01 it costs 0.6ms for 1,200 characters and 1.6ms at the 3,000-character input cap (linear in length, well inside a keystroke).
