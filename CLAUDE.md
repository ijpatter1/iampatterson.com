...
- **10b Core Web Vitals & Performance** 🔄 on branch `phase/10b-core-web-vitals` cut from `main@8e902c5`. D1a ✅ CWV measurement foundation (`6df76a7`). D1b ✅ Lighthouse baseline (`36e642d` + `83b4157`) — mobile + desktop presets via `scripts/capture-cwv-baseline.sh`; **desktop Perf 99-100 across all three dynamic routes, LCP 0.7-0.8s**; mobile-throttled Perf 80/0/75 (expected given Slow-4G + 4× CPU emulation). D1c ✅ investigation closed (`c2de9f6` + `83b4157`) — desktop is portfolio-grade, mobile bounded-and-known with carry-forward levers documented. **D1 declared sufficiently measured + landed; mobile optimization deferred to post-field-data when the sGTM trigger + BQ tag for `web_vital` catch up.** Remaining: D5 WebSocket connection reliability (reconnect backoff + graceful degradation), D6 overlay render perf profile under ~100-event session.
