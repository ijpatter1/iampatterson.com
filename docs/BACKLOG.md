...
- **No Metabase card uses `is_synthetic`** — 45 cards, zero reference it. [14.6] made the dimension correct and nothing consumes it, so every dashboard still mixes ~193k generated sessions with ~90 real ones undifferentiated. On a site whose pitch is the instrumentation, this is the most visible gap. One filter or breakout on an existing card. _(session-2026-09-09-001, found by converting a rubber-stamp UAT gate to a real check)_
